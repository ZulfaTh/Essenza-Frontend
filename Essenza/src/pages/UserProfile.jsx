...
export default function UserProfile() {
  const { backendUrl, userData, token, getUserData, setUserData,getStaffsData } = useContext(AppContent);
  ...
  const getUserAppointments = async () =>{
    try {
      ...
      if(data.success){
        ...
        console.log("Appointments:", data.appointments, Array.isArray(data.appointments));
        ...
        <div className="w-44 h-44 rounded-full overflow-hidden border-4 border-purple-400">
          {isEdit ? (
            <label htmlFor="image" className="cursor-pointer w-full h-full block">
              <img
                src={image ? URL.createObjectURL(image) : userData.image || "/profile.jpg"}
                alt="Profile"
        ...
        <div className="space-y-4">
             ...
             {appointments.map((item,index)=>(
              <div className="grid grid-cols-[1fr_2fr] gap-4 sm:flex sm:gap-6 py-2 " key={index}>
                <div>
                  <img className="w-32 bg-indigo-50" src={item.staffData.image} alt=""/>
                </div>
                <div className="flex-1 text-sm text-zinc-600">
                  <p className="text-neutral-800 font-semibold">Service By: {item.staffData.name}</p>
...
</div>                     <p className="text-xs mt-1"><span className="text-sm text-neutral-700 font-medium">Date & Time : </span>{slotDateFormat(item.slotDate)} | {item.slotTime}</p>
                </div>
                <div></div>
                <div className="flex flex-col gap-2 justify-end">
                  {!item.cancelled &&  <button className="text-sm text-stone-500 text-center sm:min-w-48 py-2 border rounded hover:bg-blue-600 hover:text-white transition-all duration-300">Payment</button>}
                {!item.cancelled &&   <button onClick={()=>cancelAppointment(item._id)} className="text-sm text-stone-500 text-center sm:min-w-48 py-2 border rounded  hover:bg-red-600 hover:text-white transition-all duration-300">Cancel</button>}
